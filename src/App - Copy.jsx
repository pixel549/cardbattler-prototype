...
import { getAIAction } from './game/aiPlayer';
import { decodeDebugSeed, randomDebugSeed } from './game/debugSeed';
...
function App() {
  ...
  // ── Debug seed state ─────────────────────────────────────────────────────
  const [debugSeedInput, setDebugSeedInput]         = useState('');
  const [randomizeDebugSeed, setRandomizeDebugSeed] = useState(false);
  ...
  useEffect(() => {
    if (!aiEnabled || !data || !state) return;

    const timer = setTimeout(() => {
      const mode = state.mode;

      // Detect mode transitions for per-combat stats
      const prevMode = prevModeRef.current;
      prevModeRef.current = mode;

      if (prevMode === 'Combat' && mode !== 'Combat' && combatStatsRef.current) {
        // Combat just ended — finalise the encounter entry
        const c = state.combat;
        combatStatsRef.current.result   = c?.victory ? 'win' : 'loss';
        combatStatsRef.current.hpAfter  = state.run?.hp ?? 0;
        // push entry into runHistory's pending list (stored separately)
        pendingEncountersRef.current.push({ ...combatStatsRef.current });
      ...
      if (mode === 'Combat' && state.combat && !state.combat.combatOver) {
        // Track turns inside combat
        if (combatStatsRef.current) combatStatsRef.current.turns = state.combat.turn ?? 0;
      }
      ...
      if (mode === 'GameOver') {
        ...
        const seed    = Date.now();
        const next    = dispatchWithJournal(initial, data, { type: 'NewRun', seed, debugSeed: nextDebugSeed });
        setState(next);
        prevModeRef.current = null;
        return;
      }

      const action = getAIAction(state, data);
      if (action) setState(prev => {
  ...
  }, [state, aiEnabled, aiSpeed]);

  // Pending encounter log for the in-progress run (cleared each new run above)
  const pendingEncountersRef = useRef([]);
  ...
  useEffect(() => {
    ...
    const filename   = `ai_runs_${pad(batchStart)}-${pad(batchEnd)}_${Date.now()}.json`;
  ...
  function exportRunData() {
    ...
    const filename = total > 0
      ? `ai_runs_${pad(1)}-${pad(total)}_${Date.now()}.json`
      : 'ai_run_data.json';
  ...
  const startNewRun = (overrideDebugSeed) => {
    ...
    const newState = dispatchWithJournal(initial, data, { type: 'NewRun', seed, debugSeed });
    setState(newState);
  };
  ...
  switch (state.mode) {
    case 'Combat':
      content = <CombatScreen state={state} data={data} onAction={handleAction} />;
      break;
    case 'Map':
  ...
  return (
    <>
      ...
      <AIDebugPanel
        ...
        onRandomizeSeed={() => setDebugSeedInput(String(randomDebugSeed()))}
      />
    </>
  );
}
