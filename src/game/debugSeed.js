 ...
 * overrides from wide continuous ranges. Same seed → same configuration every
 * time, but the value space covers the entire playable range for every variable.
 *
 * Variables generated per seed:
 *   playerMaxHP        10 – 500  (log-uniform)
 *   startingCardCount  1  – 100  (log-uniform)
 *   enemyHpMult        0.1– 10x  (log-uniform)
 *   enemyDmgMult       0.1– 10x  (log-uniform)
 *   playerMaxRAM       1  – 32   (log-uniform)
 *   drawPerTurnDelta   -4 – +12  (uniform)
 *   actOverride        null | 1–7 (35 % null)
 *   encounterKind      null | normal | elite | boss
 *   enemyCount         1–5       (how many enemies to pull from full pool)
...
// ---------------------------------------------------------------------------
// Core decode — seed → override object
// ---------------------------------------------------------------------------
export function decodeDebugSeed(seed) {
  ...
  const actRoll     = n();
  // 35 % chance of null (use real act), else pick 1–7
  ...
  // Enemy count (1–5) + a seed to pick which enemies from the full roster
...
export function describeDebugSeed(ovr) {
  ...
  parts.push(`${ovr.enemyCount}×rnd`);  // enemy count always shown (always from full pool)
  return parts.join(' · ');
}

// ---------------------------------------------------------------------------
// Sensible decode — same structure, tighter ranges tuned for survivable runs
//
//   playerMaxHP        45  – 150   (vs wild 10–500)
//   startingCardCount  12  – 25    (vs wild 1–100)  — cuts deck-bloat extremes
//   enemyHpMult        0.65– 1.8x  (vs wild 0.1–10x) — 3x was one-shotting turn 1
//   enemyDmgMult       0.65– 1.5x  (vs wild 0.1–10x) — 1.72x already lethal
//   playerMaxRAM       5   – 14    (vs wild 1–32)
//   drawPerTurnDelta   -1  – +4    (vs wild -4–+12)  — removes worst deck-stall combos
...
//
// seed 8194  → HP:411  Cards:3   EnemyHP:0.41x  EnemyDMG:1.81x  RAM:16  Draw:-2  Act:3   Kind:normal  5×rnd
// seed 6146  → HP:14   Cards:95  EnemyHP:0.22x  EnemyDMG:1.32x  RAM:7   Draw:+2  Act:null Kind:null   4×rnd
// seed 48    → HP:63   Cards:82  EnemyHP:2.84x  EnemyDMG:0.73x  RAM:7   Draw:-3  Act:null Kind:elite  3×rnd
// seed 0     → HP:62   Cards:3   EnemyHP:5.65x  EnemyDMG:0.37x  RAM:2   Draw:+2  Act:4   Kind:null   2×rnd
// seed 73    → HP:61   Cards:1   EnemyHP:7.34x  EnemyDMG:6.59x  RAM:1   Draw:+1  Act:null Kind:null  2×rnd
// seed 12    → HP:62   Cards:1   EnemyHP:0.15x  EnemyDMG:0.10x  RAM:24  Draw:+2  Act:null Kind:elite 4×rnd
// seed 8     → HP:62   Cards:1   EnemyHP:0.35x  EnemyDMG:3.63x  RAM:2   Draw:-4  Act:4   Kind:elite  4×rnd
// seed 14    → HP:62   Cards:1   EnemyHP:0.30x  EnemyDMG:0.59x  RAM:1   Draw:+1  Act:5   Kind:boss   3×rnd
// seed 10    → HP:62   Cards:2   EnemyHP:0.75x  EnemyDMG:1.92x  RAM:15  Draw:-1  Act:3   Kind:elite  5×rnd
// seed 133   → HP:60   Cards:2   EnemyHP:0.35x  EnemyDMG:6.32x  RAM:2   Draw:+12 Act:7   Kind:elite  3×rnd
// ---------------------------------------------------------------------------
